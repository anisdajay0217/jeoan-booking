...
// Strip gcashScreenshot before saving to avoid QuotaExceededError
// ════════════════════════════════════════
function getArchive() {
...
function saveArchive(arr) {
  try {
    sessionStorage.setItem('jb_archive', JSON.stringify(arr));
  } catch (e) {
    // If quota exceeded, try removing oldest entries until it fits
    if (e.name === 'QuotaExceededError' || e.code === 22) {
      const trimmed = arr.slice(0, Math.max(arr.length - 1, 0));
      if (trimmed.length < arr.length) {
        try { sessionStorage.setItem('jb_archive', JSON.stringify(trimmed)); } catch {}
      }
    }
  }
}
...
function archiveBooking(booking) {
  ...
  if (!arr.some(b => String(b.id) === String(booking.id))) {
    // Strip gcashScreenshot — base64 images blow past the ~5MB sessionStorage limit
    const { gcashScreenshot, ...bookingWithoutScreenshot } = booking;
    arr.unshift({ ...bookingWithoutScreenshot, archivedAt: new Date().toISOString() });
    saveArchive(arr);
  }
}
...
function generateFullyPaidReceipt(b) {
  ...
  ctx.save();
  ctx.fillStyle = mid; ctx.globalAlpha = 0.06;
  for (let x = P; x < D - P; x += 14)
    for (let y = 60; y < D - 60; y += 14) {
  ...
  cy += 14;
  ctx.save();
  ctx.fillStyle = greenPale; ctx.strokeStyle = '#a8d5bc'; ctx.lineWidth = 1;
  rrect(P, cy - 12, W, 30, 10); ctx.fill(); ctx.stroke(); ctx.restore();
  ...
  terms.forEach(term => {
    ...
    for (const w of words) {
      const test = (broken ? l2 : l1) + w + ' ';
      if (!broken && ctx.measureText(test).width > W - 18) { broken = true; l2 = w + ' '; }
      else if (broken) { if (ctx.measureText(l2 + w + ' ').width < W - 18) l2 += w + ' '; }
      else { l1 = test; }
    }
    txt(l1.trim(), P + 13, cy, '400 9.5px Arial,sans-serif', '#5a2535');
...
async function markDone(id) {
  try {
    ...
    showToast('❌ Could not archive. Storage may be full.');
  }
}
